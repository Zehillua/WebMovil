            ...
            {comidas.length > 0 && (
              <>
                <div className="carrito-section-header">
                  <h3>🍽️ Comidas (${totalComidas.toLocaleString('es-CL')})</h3>
                </div>
                {comidas.map((comida) => (
                  <div className="carrito-item" key={comida._id}>
                    <div className="carrito-img">
                      {/* Usar un emoji o un icono SVG si es posible */}
                    ...
                    <div className="carrito-info">
                      ...
                      <div className="carrito-precio-unit">
                        Precio unitario: ${comida.precio.toLocaleString('es-CL')}
                      </div>
                    ...
                    <button
                      ...
                      title="Eliminar"
                    >
                      {/* Usar emoji en lugar de imagen externa */}
                      🗑️
                    </button>
                  </div>
                ))}
            ...
            {promociones.length > 0 && (
              <>
                <div className="carrito-section-header promo-section">
                  <h3>🎉 Promociones (${totalPromociones.toLocaleString('es-CL')})</h3>
                </div>
                {promociones.map((promocion) => (
                  <div className="carrito-item promocion-item" key={promocion._id}>
                    <div className="carrito-img">
                      {/* Usar un emoji o un icono SVG si es posible */}
                      <span className="promo-main-icon">🎁</span>
                    ...
                    <div className="carrito-info">
                      ...
                      <div className="carrito-precio-unit promo-precio">
                        Precio promocional: ${promocion.precio.toLocaleString('es-CL')}
                      </div>
                      ...
                      <div className="carrito-total promo-total">
                        Total: ${(promocion.precio * promocion.cantidad).toLocaleString('es-CL')}
                      </div>
                    </div>
                    <button
                      className="carrito-eliminar"
        ...
        <div className="carrito-footer">
          ...
          <div className="carrito-total-footer">
            <div className="total-desglose">
              {totalComidas > 0 && (
                <div className="subtotal-line">Comidas: ${totalComidas.toLocaleString('es-CL')}</div>
              )}
              {totalPromociones > 0 && (
                <div className="subtotal-line promo-line">Promociones: ${totalPromociones.toLocaleString('es-CL')}</div>
              )}
            </div>
            <div className="total-principal">
              <span>Total:</span>
              <span className="carrito-total-num">${totalBack.toLocaleString('es-CL')}</span>
            </div>
          </div>
        </div>
      )}
      ...
      {showPago && (
        <div className="modal-pago-overlay" onClick={() => setShowPago(false)}>
          <div className="modal-pago" onClick={e => e.stopPropagation()}>
              ...
              <div style={{ margin: '1rem 0' }}>
                <label>
                  ...
                  <input
                    ...
                    className="modal-input" // Usar una clase para estilizar
                ...
                <label style={{ marginLeft: 12 }}>
                  ...
                  <input
                    ...
                    className="modal-input small-input" // Usar clases para estilizar
                  />
                </label>
              </div>
            )}
                  ...
                  <div style={{ margin: '0.7rem 0' }}>
                    <label>
                      ...
                      <input
                        ...
                        className="modal-input tip-input" // Usar clases para estilizar
                        disabled={!propina}
                      />
                    </label>
                  </div>
            ...
            <div className="modal-desglose-total">
              {totalComidas > 0 && (
                <div><strong>Comidas:</strong> ${totalComidas.toLocaleString('es-CL')}</div>
              )}
              {totalPromociones > 0 && (
                <div><strong>Promociones:</strong> ${totalPromociones.toLocaleString('es-CL')}</div>
              )}
              {esDelivery && propina && cantidadPropina && (
                <div><strong>Propina:</strong> ${Number(cantidadPropina).toLocaleString('es-CL')}</div>
              )}
              <hr />
