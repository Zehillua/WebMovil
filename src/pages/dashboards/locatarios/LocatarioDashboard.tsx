...
const LocatarioDashboard: React.FC = () => {
  ...
  const handleAgregarPromocion = async (e: React.FormEvent) => {
    ...
    if (precioPromo >= precioOriginalTotal) {
      alert(`El precio de la promoción ($${precioPromo}) debe ser menor al precio original total ($${precioOriginalTotal})`);
      return;
    }
      ...
      <div className="main-actions">
        {vistaActual === 'productos' ? (
          <>
            ...
            <button onClick={() => setVistaActual('promociones')}>Ver Promociones</button>
          </>
        ) : (
          <>
            <button onClick={() => setMostrarFormularioPromo(true)}>Agregar Promoción</button>
            <button onClick={() => setVistaActual('productos')}>Ver Productos</button>
          </>
        )}
                    ...
                    <div className="selected-items">
                        ...
                        <div key={idx} className="selected-item">
                          <div className="item-info">
                            ...
                            <span className="item-details">
                              x{comida.cantidad} • ${(comida.precioOriginal * comida.cantidad).toLocaleString()}
                            </span>
                          </div>
                          <button 
                            type="button" 
                      ...
                      <div className="price-summary">
                        <div className="original-price">
                          ...
                          <strong>${calcularPrecioOriginalTotal().toLocaleString()}</strong>
                        </div>
                        {formularioPromo.precio && (
                          <div className="promo-price">
                            <span>Precio promocional:</span>
                            <strong className="promo-value">${parseFloat(formularioPromo.precio || '0').toLocaleString()}</strong>
                          </div>
                        )}
                        {formularioPromo.precio && calcularPrecioOriginalTotal() > 0 && (
                          <div className="savings">
                            ...
                            <strong className="savings-value">
                              ${(calcularPrecioOriginalTotal() - parseFloat(formularioPromo.precio || '0')).toLocaleString()}
                            </strong>
                          </div>
                        )}
                      </div>
        ...
        <div className="overlay selector-overlay" onClick={() => setMostrarSelectorComidas(false)}>
          <div className="selector-comidas" onClick={(e) => e.stopPropagation()}>
            ...
            >
              ...
              Finalizar Selección 
              {formularioPromo.comidasSeleccionadas.length > 0 && 
                `(${formularioPromo.comidasSeleccionadas.length} seleccionadas)` // ✅ CORREGIDO
              }
            </button>
